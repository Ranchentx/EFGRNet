...
def adjust_learning_rate_m2det(optimizer, epoch, step_epoch, gamma, epoch_size,
                         iteration, cfg):
    if epoch <= cfg.TRAIN.WARMUP_EPOCH:
        iteration += (epoch_size * (epoch - 1))
        lr = 1e-6 + (cfg.SOLVER.LR[0]-1e-6)\
         * iteration / (epoch_size * cfg.TRAIN.WARMUP_EPOCH)
    else:
        for i in range(len(step_epoch)):
            if step_epoch[i]>=epoch:
                lr = cfg.SOLVER.LR[i]
                break
...
def train(train_loader, net, criterion, optimizer, epoch, epoch_step, gamma,
    ...
    for iteration, (imgs, targets, _) in enumerate(train_loader):
        t0 = time.time()
        lr = adjust_learning_rate_m2det(optimizer, epoch, epoch_step, gamma,
                                  epoch_size, iteration, cfg)
        imgs = imgs.cuda()
        imgs.requires_grad_()
        with torch.no_grad():
...
def eval_net(val_dataset,
    ...
    for idx, (imgs, _, img_info) in enumerate(val_loader):
        with torch.no_grad():
            ...
            forward_time = t4 - t1
            if idx % 10 == 0:
                print('im_detect: {:d}/{:d} {:.6f}s {:.6f}s {:.6f}s'.format(
...
def main():
    ...
    for epoch in range(start_epoch + 1, end_epoch + 1):
        ...
        train(train_loader, net, criterion, optimizer, epoch, epoch_step,
              gamma, end_epoch, cfg)
        if (epoch == 10) or (epoch % 5 == 0 and epoch >90):
            save_checkpoint(net, epoch, size, optimizer)

        if (epoch % 40 == 0) or (epoch % 10 ==0 and epoch >= 120):
            eval_net(
                val_dataset,
                val_loader,
                net,
