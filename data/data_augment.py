...
class BaseTransform_img(object):
    ...
    def __call__(self, img, target=None):
        ...
        interp_method = interp_methods[0]
        img = cv2.resize(
            np.array(img), (int(self.resize_wh), int(self.resize_wh)),
...
class BaseTransform_ration(object):
    """Defines the transformations that should be applied to test PIL image
        for input into the network
    ...
    def __call__(self, img, target=None):
        ...
        interp_method = interp_methods[0]
        img = cv2.resize(
            np.array(img), None, None, fx=self.resize_wh[0], fy = self.resize_wh[1],
            interpolation=interp_method).astype(np.float32)
        img -= self.means
        img = img.transpose(self.swap)
        return torch.from_numpy(img)
