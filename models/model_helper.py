...
def trans_layers_bn(block, fpn_num):
    layers = list()
    for i in range(fpn_num):
        layers += [nn.Sequential(BasicConv(block[i], 256, kernel_size=3, stride=1, padding=1),
...
def latent_layers_bn(fpn_num):
    layers = []
    for i in range(fpn_num):
        layers += [BasicConv(256, 256, kernel_size=3, stride=1, padding=1)]
    return layers
...
def up_layers(fpn_num):
    layers = []
    for i in range(fpn_num - 1):
        layers += [nn.ConvTranspose2d(256,256, kernel_size=4, stride=2, padding=1)]
    return layers
...
class FpnAdapter_bn(nn.Module):
    def __init__(self, block, fpn_num):
        super(FpnAdapter_bn, self).__init__()
        self.trans_layers = nn.ModuleList(trans_layers_bn(block, fpn_num))
        self.up_layers = nn.ModuleList(up_layers(fpn_num))
        self.latent_layers = nn.ModuleList(latent_layers_bn(fpn_num))
        self._init_modules()
    ...
    def forward(self, x):
        ...
        for (p, t) in zip(x, self.trans_layers):
            trans_layers_list.append(t(p))
        last = self.latent_layers[-1](trans_layers_list[-1])
        # last layer
        fpn_out.append(last)
        _up = self.up_layers[-1](last)
        for i in range(len(trans_layers_list) - 2, -1, -1):
            q = trans_layers_list[i] + _up
            q = self.latent_layers[i](q)
            fpn_out.append(q)
            if i > 0:
                _up = self.up_layers[i - 1](q)
        fpn_out = fpn_out[::-1]
...
class FpnAdapter_cat(nn.Module):
    def __init__(self, block, fpn_num):
        super(FpnAdapter_cat, self).__init__()
        self.trans_layers = nn.ModuleList(trans_layers_1(block, fpn_num))
        self.up_layers = nn.ModuleList(up_layers(fpn_num))
        self.latent_layers = nn.ModuleList(latent_layers_1(fpn_num))
        self._init_modules()
    ...
    def forward(self, x):
        ...
        _up = self.up_layers[-1](last)
        for i in range(len(trans_layers_list) - 2, -1, -1):
            q = torch.cat([trans_layers_list[i], _up], 1)
            q = self.latent_layers[i](q)
            fpn_out.append(q)
            if i > 0:
                _up = self.up_layers[i - 1](q)
